/* EVDB JS API (EVDB.API) 0.4.1 / develop@eventful.com / 2016-01-22 See http://api.eventful.com/js/api.changes for a changelog. 

Instructions: - Include the EVDB JS API script in your HTML document like so: 
- Call the EVDB.API.call() function as specified below. 
Format of API Call: EVDB.API.call(sMethod, args, fnCallback); 
- sMethod is the API method to call. see: http://api.eventful.com/docs 
...
- The only argument to the function will be the data passed back from the API in JS data structures (objects, arrays, etc.). 
...
EVDB.API.requests = [];
EVDB.API.elHead = null; var proto;

/* Either http or https if we can discover via the window.location.protocol field */
    ...
    try {
        ...
        if (typeof args == "object") {
            ...
            if (EVDB.API.user_key)
                oUserArgs.user_key = EVDB.API.user_key;
